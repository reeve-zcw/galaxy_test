...
line_bot_api = LineBotApi(
    'qCdIq5FxMn9ktvMCzGllh8PVrX5yIO9xfEZwP3DOr0WP9hmNOWvZ2zIn4TVvsCmZsxfui0LRbibkEaTjTPHu3p7VDr6cjnQeZtoGXG/VVCd6Wv6kRU2RD9owZRmIGTO3j5JeFsQJDL60NSRzdun/3QdB04t89/1O/w1cDnyilFU=')
...
def handle_message(event):
    ...
    if calendar_state:
        ...
        line_bot_api.reply_message(event.reply_token, TextSendMessage(
            'https://notify-bot.line.me/oauth/authorize?response_type=code&client_id=iQgYatSYppkaZb4uobXjxS&redirect_uri=https://01b1-2402-7500-4e6-d672-c7e-26c2-f309-5359.jp.ngrok.io&scope=notify&state=NO_STATE'))
        ...
        try:
            ...
            userInfo = get_userInfo(user_name)
            global standard_lst
                ...
                for i in range(len(lst)):
                    item.append(QuickReplyButton(
                        action=MessageAction(label=standard_lst[i].text.split('\n')[0],
                                             text=standard_lst[i].text.split('\n')[0])))
                message = TextSendMessage(
                    text='請選擇系組',
                    quick_reply=QuickReply(
                        items=item))
...
第十四任   王錫福"""))
        ...
        if '：' not in mtext:
            ...
            line_bot_api.reply_message(event.reply_token, [TextMessage(text=show_str),
                                                           TextMessage(
                                                               text='此資訊僅做為參考，空教室亦可能供課程外租借使用，請依實際情況為主！')])
